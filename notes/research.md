...
- Asyncio: `QueuePool` uses a `threading.Lock`, so it's incompatible; async engines
...
- Object states: transient, then pending (`add()`), then persistent (flush), then detached (close).
- `commit()` = flush + COMMIT; autoflush runs before queries. Flush is not commit.
...
- Lazy (`select`) is the default, which causes the N+1 problem.
...
- Fixes: eager load (`selectinload`), the `AsyncAttrs` mixin (`await obj.awaitable_attrs.x`),
...
- **asyncpg + pgbouncer trap**: transaction-mode pgbouncer breaks asyncpg's prepared
  statement cache, causing `DuplicatePreparedStatementError`. Fix: `statement_cache_size=0`
